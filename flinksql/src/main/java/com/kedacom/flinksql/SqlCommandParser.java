...
public final class SqlCommandParser {
    ...
    public static List<SqlCommandCall> parse(List<String> lines) {
        ...
        for (String line : lines) {
            ...
            stmt.append("\n").append(line);
            if (line.trim().endsWith(";") && !line.trim().contains("--")) {
    ...
    public enum SqlCommand {
        INSERT_INTO(
                "(" +
                        "((INSERT)|(insert))\\s+((INTO)|(into)|(OVERWRITE)|(overwrite)).*" +
                        ")",
                SINGLE_OPERAND),

        CREATE_TABLE(
                "(" +
                        "((CREATE)|(create))\\s+((TABLE)|(table)).*" +
                        ")",
                SINGLE_OPERAND),
        ...
        SET(
                ...
                (operands) -> {
                    ...
                    return Optional.of(new String[]{operands[3], operands[4]});
                });
