...
public class ProtobufDecoderFlinkOperator extends AbstractFlinkOperator
{
    private ElementDescriptor desc = CommonUtils.createElementDesc(
            "flinktransform_protobufdecoder",
            "protobufdecoder transform",
            CommonUtils.Transform,
            CommonUtils.readfile("jsonfilesgraph/protobufdecoderspec.json"),
            Arrays.asList(
                    new Elesingleportinfo(
                            0L,
                            Arrays.asList(CommonUtils.bytearray, CommonUtils.kafkafulldata),
                            "input 0",
                            "dataoinput"
                    )
            ),
            Arrays.asList(
                    new Elesingleportinfo(
                            0L,
                            Arrays.asList(CommonUtils.protobufmessage),
                            "output 0",
                            "dataoutput"
                    )
            )
    ...
    {
        return source.flatMap(new ProtobuftoMapFlatmap(transconfig)).returns(Types.GENERIC(Object.class));
    }
    ...
    {
        return trans.flatMap(new ProtobuftoMapFlatmap(transconfig)).returns(Types.GENERIC(Object.class));
    }*/
    ...
    {
        return trans.flatMap(new ProtobufDecoderFlatmap(transconfig)).returns(Types.GENERIC(Object.class));
    }
}
