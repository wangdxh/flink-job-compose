...
public class RedisSinkFlinkOperator extends AbstractFlinkOperator
{
    private ElementDescriptor desc = CommonUtils.createElementDesc(
            "flinksink_redis",
            "redis element",
            CommonUtils.Sink,
            CommonUtils.readfile("jsonfilesgraph/redissinkspec.json"),
            Arrays.asList(
                    new Elesingleportinfo(
                            0L,
                            Arrays.asList(CommonUtils.listobject),
                            "input 0",
                            "datainput"
                    )
            ),
