...
public class JdbcSourceFlinkOperator extends AbstractFlinkOperator
{
    private ElementDescriptor desc = CommonUtils.createElementDesc(
            "flinksource_jdbc",
            "jdbc source",
            CommonUtils.Source,
            CommonUtils.readfile("jsonfilesgraph/jdbcsourcespec.json"),
            Arrays.asList(),
            Arrays.asList(
    ...
    {
        return env.addSource(new JdbcSource(src));
    }
}
